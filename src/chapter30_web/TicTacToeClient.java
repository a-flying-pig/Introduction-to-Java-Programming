...
public class TicTacToeClient extends JApplet implements Runnable, TicTacToeConstants {
	...
	private JLabel jlblTilte = new JLabel();
	...
	public void init() {
		...
		jlblTilte.setHorizontalAlignment(JLabel.CENTER);
		jlblTilte.setFont(new Font("SansSerif", Font.BOLD, 16));
		jlblTilte.setBorder(new LineBorder(Color.black, 1));
		jlblStatus.setBorder(new LineBorder(Color.black, 1));
		
		// Place the panel and the labels to the applet
		add(jlblTilte, BorderLayout.NORTH);
	...
	private void connectToServer() {
		try {
			...
			// create an output stream to receive data from the server
	...
	public void run() {
		...
		try {
			...
			if (player == PLAYER1) {
				myToken = 'X';
				otherToken = 'O';
				jlblTilte.setText("Player 1 with token 'X");
				jlblStatus.setText("Waiting for player 2 to join");
				
				// Receive startup notification from the server
				fromServer.readInt(); // What ever read is ignored
				...
				myToken = 'O';
				otherToken = 'X';
				jlblTilte.setText("Player 2 with token 'O");
